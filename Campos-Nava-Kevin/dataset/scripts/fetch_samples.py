...
Uso:
    ...
    python3 dataset/scripts/build_dataset.py --families Mirai Gafgyt Tsunami

    # 2) baja las muestras Intel del catálogo (por defecto, máx. 10):
    python3 dataset/scripts/fetch_samples.py --limit 10

    # o salta el catálogo y consulta familias al vuelo:
    python3 dataset/scripts/fetch_samples.py --families Mirai --limit 5
"""
import argparse
import csv
import hashlib
...
# El script vive en dataset/scripts/; el catálogo vive en dataset/ y la raíz del
# repo (para samples/quarantine) está un nivel más arriba.
...
def main():
    ...
    parser.add_argument(
        "--catalog", default=os.path.join(DATA_DIR, "botnet_elf.json"),

...
def stats() -> dict:
    """Métricas agregadas para la página de Estadísticas."""
    with _conn() as c, c.cursor() as cur:
        cur.execute("SELECT count(*), coalesce(sum(size), 0) FROM samples")
        n_samples, bytes_total = cur.fetchone()

        cur.execute("SELECT count(*) FROM reports")
        n_reports = cur.fetchone()[0]

        cur.execute("SELECT kind, count(*) FROM reports GROUP BY kind ORDER BY count(*) DESC")
        por_comando = [{"kind": k, "total": n} for k, n in cur.fetchall()]

        cur.execute(
            "SELECT sha256, filename, size, uploaded_at FROM samples "

...
Uso:
    python3 kali/dynamic.py <ruta_local> [segundos_ejecucion]
...
Requiere que la VM tenga procdump instalado, ptrace_scope=0 y sudo NOPASSWD
para el usuario `kali` (ver vagrant/provision_kali.sh o el readme).
"""
import os
import subprocess
import sys
...
from core.ssh import conectar, subir


def _vm_corriendo(name: str) -> bool:
    out = subprocess.run(
...
def iniciar_vm(name: str) -> None:
    if _vm_corriendo(name):
        print(f"[*] VM '{name}' ya está encendida")
        return
    print(f"[*] Encendiendo VM '{name}' (headless)...")
...
def aislar_host() -> None:
    """Aplica el firewall que aísla el host de la VM (kali/isolate_host.sh).
    ...
    if "vboxnet0" not in hostonly:
        ...
        return
    script = Path(__file__).parent / "isolate_host.sh"
    ...
    if r.returncode != 0:
        raise RuntimeError(
            ...
            "Corre 'bash kali/isolate_host.sh' a mano y reintenta."
        )
...
def analizar(local_path: str, segundos: int = 20) -> None:
    ...
    iniciar_vm(name)
    client = esperar_ssh(c["host"], c["port"], user, c["key_path"])
    try:
        ...
        print("    (la VM sigue encendida; apágala con 'kali/cli.sh stop kali' si quieres)")
    finally:
        client.close()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python3 kali/dynamic.py <ruta_local> [segundos_ejecucion]")
        sys.exit(1)
    ruta = sys.argv[1]
